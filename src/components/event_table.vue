<template>
    <section class="event-all" :class="{'event-all-show':isShow}">
        <div class="table-box">
            <div class="edit-input" :class="{'edit-input-show':is_edit}" @keyup.enter="editData">
                <input type="text" ref="content" v-model="info.content">
            ...
            <div class="screen-box">
                ...
                <input type="text" class="div-search"  placeholder="筛选关键词" v-model="screen_title">
            </div>
...
<script>
    export default {
        ...
        computed: {
            myNotes() {
                var self = this;
                return self.$store.state.events.filter(function(d){
                    if(self.screen_type !== 0 && self.screen_title === ''){ // 选了类型但是没有筛选关键字
                        if( d.type === self.screen_type ){
                            return d;
                        }
                    }else if(self.screen_type !== 0 && self.screen_title !== ''){ // 选了类型同时有筛选关键字
                        if( d.type === self.screen_type && d.content.indexOf(self.screen_title) !== -1){
                            return d;
                        }
                    }else if(self.screen_type === 0 && self.screen_title !== ''){ // 没有类型，只有筛选关键字
        ...
        methods: {
            ...
            deleteDialog(index,id){
                this.$emit('deletedialog',index,id)
            },
            doSelect(type){
            ...
            showInput(index){
                ...
                this.info = {
                    index: index,
                    content: this.myNotes[index].content,
                    id: this.myNotes[index].id
                }
